...
并将每种特征分门别类的存入这份feed_dict字典中。对应的键名字即为特征的名字。
这样一来，feed_dict这一份样本字典中除了原始的user_id,item_id,label以外，还存入了相应的其他上下文的补充特征。
注：feed_dict这一份样本字典中,这一个用户对应的物料item可能是一个，也可能是多个。该函数给出了应对这两种情况的取物料上下文特征的处理方式。
"""
def get_context_feature(feed_dict, index, corpus, data):
	...
	总结：在下面这一段分支里，只可能走else: for item list这一分支的处理
	"""
...
class ContextCTRModel(CTRModel):
	...
	class Dataset(CTRModel.Dataset):
		# 含有基础两种id和label以及扩充的相应上下文特征的完整版的一份feed_dict
		def _get_feed_dict(self, index):
			feed_dict = super()._get_feed_dict(index)
			feed_dict = get_context_feature(feed_dict, index, self.corpus, self.data)
			return feed_dict
