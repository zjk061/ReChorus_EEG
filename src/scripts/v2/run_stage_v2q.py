"""Run Stage V2-Q context-aware margin shrinkage search."""
...
from baselines.stage_v2q import run_stage_v2q_analysis, write_stage_v2q_outputs  # noqa: E402


def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--v2o_dir", type=Path, default=ROOT / "docs/v2/stage_v2o_results")
    parser.add_argument("--report_dir", type=Path, default=ROOT / "docs/v2/stage_v2q_results")
    return parser.parse_args()


def main() -> None:
    args = arguments()
    result = run_stage_v2q_analysis(args.v2o_dir)
    write_stage_v2q_outputs(result, args.report_dir)
    ...
    print(
        ...
        f"delta_vs_v2p={decision['delta_protocol_gauc_vs_v2p']:+.6f} "
        f"fold_seed_win_rate={decision['fold_seed_delta_win_rate_vs_v2p']:.3f} "
