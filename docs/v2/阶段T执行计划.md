# 阶段 T 执行计划：冻结 EEG 候选的有限训练优化

> 阶段 G-R 已完成。正式优化对象冻结为 band/region 多尺度、候选感知 bilinear EEG residual；H2/E0、causal shuffle 和 zero 必须继续作为配对对照。本阶段只优化训练稳定性，不再选择 EEG 表示或融合结构。
...
> 在不改变已冻结 EEG 表示/融合、内容/历史合同、不使用 item ID、不访问 locked test 的条件下，用户平衡训练或同用户排序损失能否稳定提高正式 EEG 候选的 GAUC？
