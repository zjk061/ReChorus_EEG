...
4. **过拟合来自超参容量与结构容量两方面**：D 的 best_epoch=7 晚于 A/C，说明超参正则有一定效果；但 D 不是「架构小模型」，后续需在**保留 EEG** 的前提下做 DGCNN（§3.2）、EEG–情绪步内对齐（§3.3 H 系列）及**架构级简化**（§3.5 I 系列）。
